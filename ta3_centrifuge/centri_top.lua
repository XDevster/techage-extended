...
local centrifuging = techage.centrifuge.centrifuging
local get_output = techage.centrifuge.get_output
local num_recipes = techage.centrifuge.get_num_recipes
local reset_cooking = techage.centrifuge.reset_cooking
local get_ingredients = techage.centrifuge.get_ingredients
local check_if_worth_to_wakeup = techage.centrifuge.check_if_worth_to_wakeup
...
local function firebox_cmnd(pos, cmnd)
	return techage.transfer(
		...
		nil,  -- payload
		nil,  -- network
		{"techage:centri_base", "techage:centri_base_on"})
end
...
local function cooking(pos, crd, nvm, elapsed)
	if nvm.techage_state == techage.RUNNING or check_if_worth_to_wakeup(pos, nvm) then
		if firebox_has_fuel(nvm, pos) then
			local state, err = centrifuging(pos, nvm, elapsed)
			...
			crd.State:standby(pos, nvm, S("No power"))
		end
	end
end
...
local tubing = {
	...
	on_beduino_request_data = function(pos, src, topic, payload)
		if topic == 141 then  -- centrifuge Output
			local nvm = techage.get_nvm(pos)
...
local _, node_name_ta3, _ =
	techage.register_consumer("centrifuge", S("Centrifuge"), tiles, {
		drawtype = "normal",
