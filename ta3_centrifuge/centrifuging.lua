--[[
	...
	Centrifuging routines for the centrifuge
...
function techage.centrifuge.get_ingredients(pos)
...
function techage.centrifuge.check_if_worth_to_wakeup(pos, nvm)
	local inv = M(pos):get_inventory()
...
function techage.centrifuge.centrifuging(pos, nvm, elapsed)
	local inv = M(pos):get_inventory()
...
function techage.centrifuge.get_output(nvm, ingr, idx)
...
function techage.centrifuge.get_num_recipes(ingr)
	return #get_recipes(ingr)
end

function techage.centrifuge.reset_cooking(nvm)
	nvm.leftover = 0
	nvm.item_percent = 0
end


techage.recipes.register_craft_type("ta3_centrifuging", {
	description = S("TA3 Centrifuging"),
	icon = "techage_concrete.png^techage_appl_furnace.png^techage_frame_ta3.png",
	width = 2,
	height = 2,
})

function techage.centrifuge.register_recipe(recipe)
